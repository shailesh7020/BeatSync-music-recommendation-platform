...
import { Search, Music2, Sparkles, X, Server } from "lucide-react";
import { api, onBackendChange, BackendInfo } from "../services/api";
...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  useEffect(() => {
    const unsub = onBackendChange((info) => {
      setBackendInfo(info);
    });
    return unsub;
  }, []);
      ...
      <header className="sticky top-0 z-40 bg-spotify-black/95 backdrop-blur-md border-b border-spotify-divider px-4 sm:px-6 py-2.5 pt-safe">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row md:items-center justify-between gap-3">
          ...
          <div className="flex items-center justify-between gap-3 w-full md:w-auto flex-1">
            ...
            {/* Backend Status Indicator / Switcher (Mobile) */}
            <button
              onClick={() => setIsServerModalOpen(true)}
              title="Tap to switch server (Local Laptop vs Cloud)"
              className="md:hidden flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-spotify-surface border border-spotify-divider flex-shrink-0 text-[10px] font-semibold text-zinc-300 active:scale-95 transition-transform"
            >
              <span
                className={`w-2 h-2 rounded-full ${
                  backendInfo.isHealthy === true
                    ? backendInfo.isLocal
                      ? "bg-emerald-400 animate-pulse"
                      : "bg-cyan-400 animate-pulse"
                    : backendInfo.isHealthy === false
                    ? "bg-red-400"
                    : "bg-yellow-400"
                }`}
              />
              <span className={backendInfo.isLocal ? "text-emerald-300 font-bold" : "text-cyan-300 font-bold"}>
                {backendInfo.isLocal ? "Local" : "Cloud"}
              </span>
            </button>
          </div>
          ...
          <div className="flex items-center gap-2 overflow-x-auto pb-1 md:pb-0 scrollbar-none flex-shrink-0">
            ...
            <button
              onClick={() => setIsServerModalOpen(true)}
              title="Click to manage backend connection & full song downloads"
              className="hidden md:flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-spotify-surface hover:bg-zinc-800 border border-spotify-divider text-[11px] text-zinc-300 transition-colors flex-shrink-0 cursor-pointer"
            >
              <span
                className={`w-2 h-2 rounded-full ${
                  backendInfo.isHealthy === true
              ...
              />
              <span className={`font-semibold ${backendInfo.isLocal ? "text-emerald-400" : "text-cyan-400"}`}>
                {backendInfo.isLocal ? "⚡ Local (Full Songs)" : "☁️ Cloud (Render)"}
              </span>
              <Server className="w-3 h-3 text-zinc-500 ml-0.5" />
            </button>
          </div>
        </div>
      </header>
